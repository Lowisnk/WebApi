  ...
  if (req.url == "/api/bookmarks") {
    switch (req.method) {
      ...
      case "PUT":
        contact = await getPayload(req, res);
        if (contact != null)
          if (contactsRepository.update(contact)) return response(res, 204);
    ...
    if (req.url.includes("/api/bookmarks/")) {
      ...
      switch (req.method) {
        ...
        case "DELETE":
          if (contactsRepository.remove(id)) return response(res, 202);
          else return response(res, 404);
      }
    }
  }
...
const server = createServer(async (req, res) => {
  ...
  if (!CORS_Preflight(req, res))
    if (!handleContactsRequest(req, res)) response(res, 404);
});
